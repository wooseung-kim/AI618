...
class MasaCtrlControlNetPipeline(StableDiffusionControlNetPipeline):
    ...
    def __call__(
        ...
        prompt,
        *,
        controlnet_conditioning: Optional[torch.FloatTensor] = None,
        batch_size: int = 1,
        height: int = 512,
        width: int = 512,
        ...
        unconditioning: Optional[List[torch.FloatTensor]] = None,
        ...
        **kwds,
        ...
        if guidance_scale > 1.:
            uc_prompt = "" if neg_prompt is None else neg_prompt
            uc_input = self.tokenizer([uc_prompt] * batch_size, padding="max_length", max_length=77, return_tensors="pt")
            uc_embeddings = self.text_encoder(uc_input.input_ids.to(DEVICE))[0]
            text_embeddings = torch.cat([uc_embeddings, text_embeddings])
        ...
        self.scheduler.set_timesteps(num_inference_steps)
        latents_list, pred_x0_list = [latents], [latents]

        for i, t in enumerate(tqdm(self.scheduler.timesteps, desc="DDIM Sampler")):
            if ref_intermediate_latents is not None:
                lat_ref = ref_intermediate_latents[-1 - i]
                _, lat_cur = latents.chunk(2)
                latents = torch.cat([lat_ref, lat_cur])

            model_in = torch.cat([latents]*2) if guidance_scale > 1. else latents

            if unconditioning is not None and isinstance(unconditioning, list):
                _, te_cond = text_embeddings.chunk(2)
                text_embeddings = torch.cat([unconditioning[i].expand_as(te_cond), te_cond])

            if controlnet_conditioning is not None:
                cond = controlnet_conditioning.to(model_in.device)
                if guidance_scale > 1.:
                ...
                noise_pred = self.unet(model_in, t, encoder_hidden_states=text_embeddings).sample
